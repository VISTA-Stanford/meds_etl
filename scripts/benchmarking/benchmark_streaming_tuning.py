    ...
    try:
        ...
        for line in process.stdout:
            ...
            if any(keyword in stripped.lower() for keyword in 
                   ["stage", "complete", "error", "warning", "✓", "✗", "time:", "throughput"]):
...
def generate_experiments(experiment_type, baseline_params, max_cpus=None):
    ...
    if experiment_type == "workers":
        ...
        for workers, shards, chunk_rows in itertools.product(
            worker_counts, shard_counts, chunk_rows_options
        ):
            params = baseline_params.copy()
            params["workers"] = workers
            params["shards"] = shards
...
def main():
    parser = argparse.ArgumentParser(
        description="Tuning experiments for omop_streaming.py"
    )
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="Path to ETL config JSON"
    )
    
    # Experiment selection
    parser.add_argument(
        "--experiment",
        choices=["workers", "shards", "chunk_rows", "compression", "memory", 
                 "workers_and_shards", "full"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Baseline number of workers (default: 8)"
    )
    parser.add_argument(
        "--baseline_shards",
        type=int,
        default=10,
        help="Baseline number of shards (default: 10)"
    )
    parser.add_argument(
        "--baseline_chunk_rows",
        type=int,
        default=10_000_000,
        help="Baseline chunk rows (default: 10,000,000)"
    )
    ...
    parser.add_argument(
        "--verbose",
        action="store_true",
        help="Enable verbose output in experiments"
    )
    parser.add_argument(
        "--dry_run",
        action="store_true",
        ...
        for i, result in enumerate(successful_results[:5], 1):
            ...
            for key, value in result["params"].items():
                if key != "verbose":  # Skip verbose in summary
                    logger.log(f"     {key}: {value}")
            if "stats" in result:
                logger.log(f"   Output: {result['stats']['num_files']} files, "
                          f"{result['stats']['total_size_gb']:.2f} GB")
            logger.log("")
