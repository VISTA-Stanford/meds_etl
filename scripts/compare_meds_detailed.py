...
def detailed_row_comparison(dir1: Path, dir2: Path, name1: str = "omop.py", name2: str = "omop_streaming.py"):
    ...
    if only_in_1:
        ...
        codes_only_1_freq = (
            df1.filter(pl.col("code").is_in(list(only_in_1)))
            .group_by("code")
            .len()
    ...
    if only_in_2:
        ...
        codes_only_2_freq = (
            df2.filter(pl.col("code").is_in(list(only_in_2)))
            .group_by("code")
            .len()
            .sort("len", descending=True)
            .collect()
    ...
    freq_comparison = (
        freq1.join(freq2, on="code", how="outer_coalesce")
        .with_columns(
            [
    ...
    subj_comparison = (
        subj_counts1.join(subj_counts2, on="subject_id", how="outer_coalesce")
        .with_columns(
            [
                pl.col("count_1").fill_null(0),
                pl.col("count_2").fill_null(0),
    ...
    example_subjects = subj_comparison.filter((pl.col("abs_diff") > 0) & (pl.col("abs_diff") < 1000)).head(1)
    ...
    if len(example_subjects) > 0:
        ...
        df1_subj = pl.scan_parquet(str(data_dir1 / "*.parquet")).filter(pl.col("subject_id") == example_subj).collect()

        df2_subj = pl.scan_parquet(str(data_dir2 / "*.parquet")).filter(pl.col("subject_id") == example_subj).collect()
        ...
        code_count_comp = (
            code_counts1.join(code_counts2, on="code", how="outer_coalesce", suffix="_2")
            .with_columns(
                [
            ...
            .with_columns([(pl.col("count_1") - pl.col("count_2")).alias("diff")])
    ...
    nv_stats1 = (
        pl.scan_parquet(str(data_dir1 / "*.parquet"))
        .select(
            [
    ...
    nv_stats2 = (
        pl.scan_parquet(str(data_dir2 / "*.parquet"))
        .select(
            [
                pl.col("numeric_value").is_not_null().sum().alias("non_null"),
                pl.col("numeric_value").is_null().sum().alias("null_count"),
    ...
    print("\n5. Recommendations:")
    print("   - Review code mapping logic in both implementations")
    print("   - Check for null/empty row filtering differences")
    print("   - Verify concept ID mapping behavior")
    print()
...
if __name__ == "__main__":
    ...
    detailed_row_comparison(baseline_dir, streaming_dir, name1="omop.py", name2="omop_streaming.py")
